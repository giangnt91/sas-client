sas
.controller('NotcallCtrl', function ($location, $scope, $rootScope, Notifi, ngDialog, $timeout, DataServices, md5, DTOptionsBuilder, DTColumnBuilder, Thesocket, SMSService) {
	// hiển thị ngày tháng
	...
	function getStudent(username, role) {
		...
		function serverData(sSource, aoData, fnCallback, oSettings) {
			...
			// if($scope.detect === undefined){
				DataServices.SearchN(role, null, null, username, start, length, search).then(function (response) {
					if (response.data.error_code === 0) {
						$scope.list_student = response.data.students;
						$scope.detect = 1;
