sas
.controller('RecallCtrl', function ($location, $scope, $rootScope, Notifi, ngDialog, $timeout, DataServices, md5, DTOptionsBuilder, DTColumnBuilder, Thesocket, SMSService) {
	// hiển thị ngày tháng
	...
	$scope.Searchwith = function () {
		...
		function serverData(sSource, aoData, fnCallback, oSettings) {
			...
			if (_Reday === '') {
				...
				Reday = convertshow(_Reday);
			}

			if (_Reday2 === '') {
				Reday2 = null;
			} else {
				Reday2 = convertshow(_Reday2);
			}
			...
			DataServices.SearchR($rootScope.auth.Role, $rootScope.auth.Username, Retime, Retime2, Reday, Reday2, Sale, start, length, search).then(function (response) {
				if (response.data.error_code === 0) {
